        ...
        <section>
          ...
          <div className="flex flex-wrap justify-around">
            <div className="max-w-xl my-3">
              <img className="rounded" src={getPrimaryProductImage(product)} onError={(event) => handleProductImageError(event, product)} alt={product.name} />
            </div>
            <div className="max-w-xl my-3">
              ...
              <p className="my-3">
                { !inCart && <button onClick={() => addToCart(product)} className={`inline-flex items-center py-2 px-5 text-lg font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 ${product.in_stock ? "" : "cursor-not-allowed"}`} disabled={ product.in_stock ? "" : "disabled" }>Add To Cart <i className="ml-1 bi bi-plus-lg"></i></button> } 
